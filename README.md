...
Этот репозиторий создан в рамках мероприятия «Хакатон по искусственному интеллекту “СИБИНТЕК‑СОФТ”». Проект занял четвёртое место в общем конкурсе.
...
При внедрении в рабочую среду рекомендуется:
...
Если нужно расширить функциональность (например, добавить новые модели, метрики или интеграции), смело редактируйте соответствующие модули. Главное — не ломайте основной пайплайн в `app/main.py` :)
